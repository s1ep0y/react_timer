...
export default class Countdown extends React.Component {
  ...
  clearTime = () => {
    const {
      timer: { active, timeAll, timeNow },
    } = this.state;
    if (active === true) {
      clearInterval(this.timerId);
      this.setState({ timer: { active: false, timeAll, timeNow } });
      return;
    }
  ...
  timeSetter = () => {
    const {
      inputs: { seconds, minutes },
      timer: { timeNow, active },
    } = this.state;
    const time = seconds + minutes * 60;
    this.setState({
      timer: {
        timeAll: time * 10,
        timeNow: timeNow === 0 ? time * 10 : timeNow,
        active,
      },
    });
  };
  ...
  minutesHandler = ({ target: { value } }) => {
    ...
    const slider = sum > 3600 ? 3600 : sum;
    this.setState({ inputs: { minutes: val, seconds, slider } });
  };
  ...
  secondsHandler = ({ target }) => {
    ...
    if (minutes === 720) {
      this.setState({ inputs: { seconds: val, minutes: 719, slider } });
      return;
    }
